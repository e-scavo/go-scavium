...
Available commands are:
	...
	cross	   [ -arch architecture ] [ -cc compiler ] [ packages... ]                          -- cross builds packages and executables
...
var (
	// Files that end up in the gscavium*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gscavium"),
	}

	// Files that end up in the gscavium-alltools*.zip archive.
...
func executableCrossPath(name string, targetos string, targetarch string) string {
	if targetos != "" {
		name += "-" + targetos
	}
	if targetarch != "" {
...
// Cross Compiling
func DoCrossInstall(cmdline []string) {
	var (
		dlgo       = flag.Bool("dlgo", false, "Download Go and build with it")
		targetarch = flag.String("targetarch", "", "Architecture to cross build for")
		targetos   = flag.String("targetos", "", "Target to cross build for")
	...
	for _, pkg := range packages {
		...
		args = append(args, "-o", executableCrossPath(path.Base(pkg), *targetos, *targetarch))
		args = append(args, pkg)
		fmt.Printf("\n\nll: %+v %+v %+v\n", gobuild.Path, args, gobuild.Env)
...
func doArchive(cmdline []string) {
	...
	var (
		...
		geth     = "gscavium-" + basegeth + ext
		alltools = "gscavium-alltools-" + basegeth + ext
	)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gscavium-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gscavium.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gscavium binary, second section holds the dev tools.
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(tc.Install(GOBIN, "golang.org/x/mobile/cmd/gomobile"))
	build.MustRun(tc.Install(GOBIN, "golang.org/x/mobile/cmd/gobind"))
...
func doXgo(cmdline []string) {
	...
	//build.MustRun(gogetxgo)
	//build.MustRun(tc.Install(GOBIN, "github.com/karalabe/xgo@latest"))
	build.MustRun(tc.Install(GOBIN, "github.com/youchainhq/xgo2@latest"))
	//build.MustRun(tc.Install(GOBIN, "github.com/2hangchen/xgo-1"))
...
func xgoTool(args []string) *exec.Cmd {
	cmd := exec.Command(filepath.Join(GOBIN, "xgo2"), args...)
	cmd.Env = os.Environ()
	cmd.Env = append(cmd.Env, []string{
		"GOBIN=" + GOBIN,
	}...)
	cmd.Env = append(cmd.Env, []string{
		"GOPATH=/home/usuario/scavo/go-scavium",
	}...)
	/*
		cmd.Env = append(cmd.Env, []string{
			"GO111MODULE=auto",
